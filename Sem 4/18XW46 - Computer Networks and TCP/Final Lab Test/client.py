...
class CustomHTTPClient(HTTPConnection):
    ...
    def get_request(self, path, send_old_date):
        print(f'Sending a GET request at {path}')
        headers = {}
        if send_old_date:
            headers['if-modified-since'] = "Thu, 22 April 2021 13:36:38 GMT+0530"
    ...
    if len(sys.argv) < 2:
        ...
        try:
            ...
            if REQUEST_METHOD.upper() == 'GET':
                x = input('Send Older Date for 304 ?(0 to indicate no, others for yes) : ')
                if x == '0':
                    client.get_request(PATH, False)
                else:
                    client.get_request(PATH, True)
